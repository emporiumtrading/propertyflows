...
import { User, Shield, Globe, Eye, Download, Trash2, KeyRound, CheckCircle2, Smartphone, Bell, X, ArrowLeft, Home, CreditCard } from "lucide-react";
...
export default function Settings() {
  ...
  const getPortalLink = () => {
    switch (user?.role) {
      case 'tenant':
        return { path: '/tenant-portal', label: 'Tenant Portal' };
      case 'landlord':
        return { path: '/owner-portal', label: 'Owner Portal' };
      case 'vendor':
        return { path: '/vendor-portal', label: 'Vendor Portal' };
      case 'admin':
        return { path: '/', label: 'Admin Dashboard' };
      case 'property_manager':
        return { path: '/', label: 'Property Management' };
      default:
  ...
  return (
    <Layout user={user}>
      ...
      <Tabs defaultValue="profile" className="space-y-6">
        <TabsList className={`grid w-full ${user?.role === 'property_manager' ? 'grid-cols-5' : 'grid-cols-4'}`}>
