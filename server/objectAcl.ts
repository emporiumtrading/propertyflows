...
class UserListAccessGroup extends BaseObjectAccessGroup {
  ...
  async hasMember(userId: string): Promise<boolean> {
    ...
    if (this.id.startsWith('lease:')) {
      const leaseId = this.id.substring(6);
      const lease = await storage.getLease(leaseId);
      if (!lease) return false;
      return lease.tenantId === userId;
    }
    
    if (this.id.startsWith('property:')) {
      const propertyId = this.id.substring(9);
...
class EmailDomainAccessGroup extends BaseObjectAccessGroup {
  ...
  async hasMember(userId: string): Promise<boolean> {
    ...
    return user.email.toLowerCase().endsWith(`@${this.id.toLowerCase()}`);
  }
}

class GroupMemberAccessGroup extends BaseObjectAccessGroup {
  constructor(id: string) {
    super(ObjectAccessGroupType.GROUP_MEMBER, id);
  }
  ...
  async hasMember(userId: string): Promise<boolean> {
    ...
    if (!validRoles.includes(this.id)) return false;
    
    return user.role === this.id;
  }
}

class SubscriberAccessGroup extends BaseObjectAccessGroup {
  constructor(id: string) {
    super(ObjectAccessGroupType.SUBSCRIBER, id);
  }

  async hasMember(userId: string): Promise<boolean> {
    const { storage } = await import('./storage.js');
    
    if (this.id === 'stripe_connected') {
      const user = await storage.getUser(userId);
      return user?.stripeOnboardingComplete === true;
    }
    ...
    if (this.id.startsWith('property_owner:')) {
      const propertyId = this.id.substring(15);
      const property = await storage.getProperty(propertyId);
      return property?.ownerId === userId;
    }
